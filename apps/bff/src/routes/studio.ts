...
import { getActiveProvider, completeProviderChat, streamProviderChat } from "../lib/providers.js";
...
async function chatJson<T = any>(system: string, user: string): Promise<T | null> {
  const p = await getActiveProvider();
  if (!p) return null;
  const r = await completeProviderChat(p, [{ role: "system", content: system }, { role: "user", content: user }]);
  if (!r.ok) return null;
...
export async function currentPersona(agent: any): Promise<PersonaSpec> {
  if (agent?.persona && agent.persona.identity) return agent.persona as PersonaSpec;
  return emptyPersona(agent?.name || "the agent", agent?.role || "", (await getCompany()).name || "the company");
}
...
export async function saveVersion(agentId: string, spec: PersonaSpec, changeNote: string, createdBy: string, parentId?: string | null): Promise<PersonaVersionRec> {
  ...
  await query(
    `INSERT INTO persona_versions (id, agent_id, number, spec, change_note, parent_id, created_by) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
...
export async function recompileGolden(agentId: string): Promise<boolean> {
  const agent = await agentRow(agentId);
  if (!agent?.golden_persona_id) return false;
  const vrow = await one<any>(`SELECT * FROM persona_versions WHERE id=$1`, [agent.golden_persona_id]);
  if (!vrow) return false;
  const instructions = compileClone(vrow.spec as PersonaSpec, playbookOf(agent), agent.name, (await getCompany()).name || "the company");
  await query(`UPDATE agents SET golden_instructions=$2 WHERE id=$1`, [agentId, instructions]);
  await query(
    `INSERT INTO settings (key, value) VALUES ('live_golden_instructions', $1) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value`,
    [JSON.stringify({ agentId, versionId: vrow.id, instructions })],
  );
  return true;
}
...
export default async function studioRoutes(app: FastifyInstance) {
  ...
  app.post("/api/rehearsal/fix", async (req, reply) => {
    ...
    if (b.route === "speech") {
      if (!b.apply) {
        const spec0 = await currentPersona(agent);
        ...
        const delta = await chatJson<PersonaDelta>(sys, user);
      ...
      const spec = await currentPersona(agent);
      ...
      void pushPersonaReload(b.agentId);
    ...
    if (!pb || !Array.isArray(pb.stages) || !pb.stages.length) {
      if (!b.apply) {
        ...
        const prop = await chatJson<{ stageId: string; stageName?: string; summary: string; before: any; after: { actions: string[]; waitBehavior: string } }>(sys, user);
      ...
      const recompiled0 = await recompileGolden(b.agentId);
      void pushPersonaReload(b.agentId);
    ...
    if (!b.apply) {
      ...
      const prop = await chatJson<{ stageId: string; summary: string; before: any; after: { actions: string[]; waitBehavior: string } }>(sys, user);
    ...
    await query(`UPDATE agents SET playbook = $2 WHERE id = $1`, [b.agentId, JSON.stringify({ kind: "calls", callPlaybook: pb })]);
    const recompiled = await recompileGolden(b.agentId);
    void pushPersonaReload(b.agentId);
  ...
  app.post("/api/rehearsal/grade", async (req, reply) => {
    ...
    await query(
      `INSERT INTO rehearsal_grades (id, call_id, agent_id, turn_seq, part, verdict, coach_ref)
  ...
  app.get("/api/rehearsal/grades", async (req, reply) => {
    ...
    const rows = await query<{ turn_seq: number; part: string; verdict: string; coach_ref: unknown }>(
      `SELECT turn_seq, part, verdict, coach_ref FROM rehearsal_grades WHERE call_id=$1 ORDER BY turn_seq`, [q.callId]);
  ...
  app.post("/api/debrief/build", async (req, reply) => {
    ...
    const j = await chatJson<any>(sys, `CALL${who ? ` (${who})` : ""}:\n${convo}`);
    ...
    await query(`INSERT INTO debriefs (id, agent_id, ref_kind, ref_id, data) VALUES ($1,$2,$3,$4,$5)`,
  ...
  app.get("/api/debrief/latest", async (req) => {
    ...
    const row = agentId
      ? await one<any>(`SELECT * FROM debriefs WHERE agent_id=$1 ORDER BY created_at DESC LIMIT 1`, [agentId])
      : await one<any>(`SELECT * FROM debriefs ORDER BY created_at DESC LIMIT 1`);
  ...
  app.post("/api/debrief/:id/finalize", async (req, reply) => {
    ...
    const row = await one<any>(`SELECT * FROM debriefs WHERE id=$1`, [id]);
  ...
  app.post("/api/clones/:agentId/sources", async (req, reply) => {
    ...
    for (const s of src) {
      ...
      if (existing) {
        ...
        await query(`INSERT INTO clone_sources (id, agent_id, title, url, transcript) VALUES ($1,$2,$3,$4,$5)`,
          [`cs_${Date.now().toString(36)}_${Math.floor(Math.random()*1e4).toString(36)}`, agentId, s.title ?? null, s.url ?? null, s.transcript]);
      }
    }
  ...
  app.get("/api/clones/:agentId/sources", async (req) => {
    const { agentId } = req.params as { agentId: string };
  ...
  app.delete("/api/clones/:agentId/sources/:sourceId", async (req, reply) => {
    const { agentId, sourceId } = req.params as { agentId: string; sourceId: string };
    await query(`DELETE FROM clone_sources WHERE agent_id=$1 AND id=$2`, [agentId, sourceId]);
  ...
  app.get("/api/clones/:agentId/goals", async (req, reply) => {
    const { agentId } = req.params as { agentId: string };
    const row = await one<{ goals: unknown }>(`SELECT goals FROM agents WHERE id=$1`, [agentId]);
  ...
  app.put("/api/clones/:agentId/goals", async (req, reply) => {
    ...
    const exists = await one<{ id: string }>(`SELECT id FROM agents WHERE id=$1`, [agentId]);
    if (!exists) return reply.code(404).send({ error: "agent not found" });
    await query(`UPDATE agents SET goals=$2 WHERE id=$1`, [agentId, JSON.stringify(goals)]);
  ...
  app.get("/api/clones/:agentId/demo-login", async (req) => {
    const { agentId } = req.params as { agentId: string };
    const row = await one<{ value: { email?: string; password?: string } }>(
      `SELECT value FROM settings WHERE key = $1`, [`demo_login:${agentId}`]);
    const own = row?.value;
    if (own && (own.email ?? "").trim()) {
      return { email: own.email ?? "", hasPassword: !!own.password, inherited: false };
    }
    // fall back to the global file
    try {
      const j = JSON.parse(readFileSync(`${AH}/gp-login.json`, "utf8")) as { email?: string; password?: string };
      return { email: j.email ?? "", hasPassword: !!j.password, inherited: true };
    } catch { return { email: "", hasPassword: false, inherited: true }; }
  });
  app.put("/api/clones/:agentId/demo-login", async (req, reply) => {
    ...
    // password is write-only: an email-only edit keeps the existing per-agent password.
    const existing = await one<{ value: { password?: string } }>(
      `SELECT value FROM settings WHERE key = $1`, [`demo_login:${agentId}`]);
    const password = (b.password ?? "").trim() || existing?.value?.password || "";
    await query(
      `INSERT INTO settings (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value`,
      [`demo_login:${agentId}`, JSON.stringify({ email, password })],
    );
    return { ok: true };
  });
  ...
  app.put("/api/clones/:agentId/playbook", async (req, reply) => {
    ...
    await query(`UPDATE agents SET playbook = $2 WHERE id = $1`, [agentId, JSON.stringify({ kind: "calls", callPlaybook: pb })]);
    const recompiled = await recompileGolden(agentId);
    void pushPersonaReload(agentId); // lands in the running session too
  ...
  async function draftFlowFromSource(agent: any, sourceId: string): Promise<CallPlaybook | null> {
    const src = await one<any>(`SELECT title, transcript FROM clone_sources WHERE agent_id=$1 AND id=$2`, [agent.id, sourceId]);
    if (!src) return null;
    const company = (await getCompany()).name || "the company";
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      j = await chatJson<any>(sys, `TRANSCRIPT "${src.title || sourceId}":\n${(src.transcript || "").slice(0, 40000)}`);
  ...
  app.post("/api/clones/:agentId/playbook/from-transcript", async (req, reply) => {
    ...
    const pb = await draftFlowFromSource(agent, b.sourceId);
  ...
  app.post("/api/clones/:agentId/persona/extract", async (req, reply) => {
    ...
    for (const r of rows.slice(0, 6)) {
      const j = await chatJson<any>(sys, `TRANSCRIPT "${r.title || r.id}":\n${(r.transcript || "").slice(0, 40000)}`);
      if (!j) continue;
      n++;
    ...
    if (!pbNow || !(pbNow.stages ?? []).length) {
      ...
      void (async () => {
        try {
          const pb = await draftFlowFromSource(agent, best.id);
  ...
  app.post("/api/sessions/:id/turns/delete", async (req, reply) => {
    ...
    await query(`DELETE FROM calibration_turns WHERE id=$1`, [row.id]);
    return { ok: true, deleted: 1 };
  });

  // -- versions --
  app.get("/api/clones/:agentId/versions", async (req) => {
    const { agentId } = req.params as { agentId: string };
    const rows = await query<any>(`SELECT id,number,change_note,created_by,created_at,parent_id FROM persona_versions WHERE agent_id=$1 ORDER BY number`, [agentId]);
    const agent = await agentRow(agentId);
    const vr = await one<any>(`SELECT value FROM settings WHERE key = $1`, [`verify_result:${agentId}`]);
    const rt = await one<any>(`SELECT value FROM settings WHERE key = $1`, [`redteam_result:${agentId}`]);
    return { versions: rows, goldenVersionId: agent?.golden_persona_id ?? null, verifyLatest: vr?.value ?? null, redteamLatest: rt?.value ?? null };
  });
  app.post("/api/clones/:agentId/versions", async (req, reply) => {
    ...
    if (b.sessionId) await query(`UPDATE calibration_sessions SET active_version_id=$2 WHERE id=$1`, [b.sessionId, v.id]);
    void pushPersonaReload(agentId); // lands in the running session too
    return { version: v };
  });
  ...
  app.post("/api/clones/:agentId/golden", async (req, reply) => {
    ...
    if (b.unpin) {
      ...
      const spec = await currentPersona(agent);
      const instructions = compileClone(spec, playbookOf(agent), agent.name, (await getCompany()).name || "the company");
      ...
      void pushPersonaReload(agentId);
      return { ok: true, unpinned: true };
    }
    const vrow = b.versionId
      ? await one<any>(`SELECT * FROM persona_versions WHERE id=$1`, [b.versionId])
      : await one<any>(`SELECT * FROM persona_versions WHERE agent_id=$1 ORDER BY number DESC LIMIT 1`, [agentId]);
    if (!vrow) return reply.code(400).send({ error: "no persona version to pin" });
    const spec = vrow.spec as PersonaSpec;
    const instructions = compileClone(spec, playbookOf(agent), agent.name, (await getCompany()).name || "the company");
    await query(`UPDATE agents SET golden_persona_id=$2, golden_instructions=$3, status='golden' WHERE id=$1`, [agentId, vrow.id, instructions]);
  ...
  app.post("/api/sessions", async (req, reply) => {
    ...
    if (!b.agentId) return reply.code(400).send({ error: "agentId required" });
    const latest = await one<any>(`SELECT id FROM persona_versions WHERE agent_id=$1 ORDER BY number DESC LIMIT 1`, [b.agentId]);
    const id = `sess_${Date.now().toString(36)}`;
    await query(`INSERT INTO calibration_sessions (id, agent_id, mode, active_version_id) VALUES ($1,$2,$3,$4)`,
  ...
  app.post("/api/sessions/:id/messages", async (req, reply) => {
    ...
    const sess = await one<any>(`SELECT * FROM calibration_sessions WHERE id=$1`, [id]);
    ...
    const spec = (vrow?.spec as PersonaSpec) ?? await currentPersona(agent);
    const system = compileClone(spec, playbookOf(agent), agent?.name || "the agent", (await getCompany()).name || "the company");
    ...
    let idx = idxRow?.m ?? 0;
    await query(`INSERT INTO calibration_turns (id,session_id,idx,role,text) VALUES ($1,$2,$3,'user',$4)`, [`t_${Date.now().toString(36)}u`, id, idx, text]);
    idx++;
    ...
    await query(`INSERT INTO calibration_turns (id,session_id,idx,role,text,version_id,latency_ms) VALUES ($1,$2,$3,'clone',$4,$5,$6)`,
  ...
  app.get("/api/sessions/:id/turns", async (req) => {
    const { id } = req.params as { id: string };
  ...
  app.post("/api/turns/:id/feedback", async (req, reply) => {
    ...
    const sess = await one<any>(`SELECT * FROM calibration_sessions WHERE id=$1`, [turn.session_id]);
    const agent = await agentRow(sess.agent_id);
    const spec = await currentPersona(agent);
  ...
  app.post("/api/clones/:agentId/apply-delta", async (req, reply) => {
    ...
    if (b.sessionId) await query(`UPDATE calibration_sessions SET active_version_id=$2 WHERE id=$1`, [b.sessionId, v.id]);
    if (b.turnId) await query(`UPDATE calibration_turns SET feedback = COALESCE(feedback,'{}'::jsonb) || $2::jsonb WHERE id=$1`, [b.turnId, JSON.stringify({ resolvedInto: v.id })]);
    return { version: v };
  });

  app.get("/api/verify/:agentId/latest", async (req) => {
    const { agentId } = req.params as { agentId: string };
    const row = await one<any>(`SELECT value FROM settings WHERE key = $1`, [`verify_result:${agentId}`]);
    return { latest: row?.value ?? null };
  });
  ...
  app.post("/api/redteam/:agentId", async (req, reply) => {
    ...
    if (!agent) return reply.code(404).send({ error: "agent not found" });
    const spec = await currentPersona(agent);
    const system = compileClone(spec, playbookOf(agent), agent.name, (await getCompany()).name || "the company");
    const provider = await getActiveProvider();
    ...
    for (const c of battery) {
      const r = await completeProviderChat(provider, [{ role: "system", content: system }, { role: "user", content: c.attack }]);
    ...
    await query(
      `INSERT INTO settings (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value`,
      [`redteam_result:${agentId}`, JSON.stringify({ average: avg, cases: results.length, at: new Date().toISOString() })],
    );
  ...
  app.post("/api/verify/:agentId", async (req, reply) => {
    ...
    for (const s of shots) {
      const r = await completeProviderChat(provider, [{ role: "system", content: system }, { role: "user", content: s.situation }]);
    ...
    await query(
      `INSERT INTO settings (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value`,
      [`verify_result:${agentId}`, JSON.stringify({ average: avg, cases: results.length, at: new Date().toISOString(), version: vrow2?.number ?? null })],
    );
    return { average: avg, results };
  });
}
