...
export default function RehearsalRoom() {
  ...
  // Only the two latest turns render inline; older ones tuck behind the "N earlier replies" chip. Resets per call.
  const [showAllTurns, setShowAllTurns] = useState(false);
  useEffect(() => { setShowAllTurns(false); }, [call?.id]);
            ...
            <div ref={feedRef} className="pds-scroll" style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "6px 16px 12px", display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              {!isZoom && rehCurrentIdx > 1 && (
                <button
                  ...
                  {showAllTurns ? "Hide earlier replies" : `${rehCurrentIdx - 1} earlier repl${rehCurrentIdx - 1 === 1 ? "y" : "ies"}`}
                </button>
              )}
              {!isZoom && (
