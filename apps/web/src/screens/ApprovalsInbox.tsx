...
// The backend stores `diff` as a single string (one change per line, each line
// prefixed with +/-/·). Parse it into signed rows for the diff/dry-run views.
function parseDiff(diff?: string): DiffRow[] {
  ...
  return diff
    ...
    .map((l): DiffRow => {
      ...
      const text = l[0] === "+" || l[0] === "-" ? l.slice(1).trim() : l;
      return [sign, text];
    });
}

function riskLabel(risk?: string): string {
  return risk === "high" ? "High risk" : risk === "medium" ? "Medium risk" : "Needs approval";
}

function ConfirmModal({ item, onClose, onConfirm }: { item: Approval; onClose: () => void; onConfirm: () => void }) {
  const diff = parseDiff(item.diff);
  return (
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ width: 500, maxWidth: "92%", borderRadius: "var(--r-lg)", background: "var(--grad-panel)", border: "1px solid color-mix(in srgb, var(--jv-red) 45%, transparent)", boxShadow: "var(--panel-shadow-active)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 11, padding: "16px 20px", borderBottom: "1px solid var(--jv-hairline)" }}>
          ...
          <div>
            ...
            <div style={{ font: "var(--fw-bold) 16px var(--font-body)", color: "var(--jv-text)", marginTop: 2 }}>{item.agent ?? "Agent"}</div>
          </div>
        </div>
            ...
            <>
              ...
              <div style={{ borderRadius: "var(--r-sm)", background: "var(--jv-void)", border: "1px solid var(--jv-border-soft)", overflow: "hidden" }}>
                {diff.map(([sign, text], i) => {
                  ...
                  return (
                    <div key={i} style={{ display: "flex", gap: 10, padding: "9px 13px", borderBottom: i < diff.length - 1 ? "1px solid var(--jv-hairline)" : "none", font: "12px/1.4 var(--font-mono)" }}>
...
function ApprovalItem({ item, onResolve }: { item: Approval; onResolve: (id: string, decision: ApprovalDecision, answer?: string) => void }) {
  const [sim, setSim] = useState(false);
  const [confirming, setConfirming] = useState(false);
  const [reply, setReply] = useState("");
  const diff = parseDiff(item.diff);
  const irreversible = item.risk === "high";
          ...
          <div style={{ display: "flex", alignItems: "flex-start", gap: 13 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 4 }}>
                <span style={{ font: "var(--fw-bold) 13px var(--font-body)", color: "var(--jv-cyan-300)" }}>{item.agent ?? "Agent"}</span>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 12 }}>
                {item.options.map((o) => (
                  <button key={o} onClick={() => onResolve(item.id, "answered", o)} style={{ padding: "8px 14px", borderRadius: "var(--r-sm)", border: "1px solid var(--jv-border-cyan)", background: "var(--grad-cyan-soft)", color: "var(--jv-cyan-300)", font: "var(--fw-semibold) 12px var(--font-body)", cursor: "pointer" }}>{o}</button>
                ))}
              </div>
              <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                <input value={reply} onChange={(e) => setReply(e.target.value)} onKeyDown={(e) => e.key === "Enter" && reply.trim() && onResolve(item.id, "answered", reply.trim())} placeholder="…or type your own answer" style={{ flex: 1, height: 38, padding: "0 13px", borderRadius: "var(--r-sm)", background: "var(--jv-void)", border: "1px solid var(--jv-border)", color: "var(--jv-text)", font: "var(--fw-medium) 13px var(--font-body)", outline: "none" }} />
                <button onClick={() => reply.trim() && onResolve(item.id, "answered", reply.trim())} style={{ width: 38, height: 38, display: "grid", placeItems: "center", borderRadius: "var(--r-sm)", border: "none", background: reply.trim() ? "var(--jv-cyan)" : "var(--jv-surface-3)", color: reply.trim() ? "var(--accent-contrast)" : "var(--jv-text-muted)", cursor: "pointer" }}><Icon name="arrow-up" size={17} /></button>
              </div>
            </div>
          </div>
        </div>
  ...
  const approve = () => { if (irreversible) setConfirming(true); else onResolve(item.id, "approved"); };
  return (
    <div style={{ borderRadius: "var(--r-md)", background: "var(--jv-surface-2)", border: `1px solid ${irreversible ? "color-mix(in srgb, var(--jv-red) 40%, transparent)" : "var(--jv-border-soft)"}`, overflow: "hidden" }}>
      <div style={{ padding: "15px 16px" }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 13 }}>
          <span style={{ width: 40, height: 40, flex: "0 0 40px", display: "grid", placeItems: "center", borderRadius: "var(--r-sm)", color: "var(--jv-cyan)", background: "rgba(41,211,245,0.08)", border: "1px solid var(--jv-border-soft)" }}><Icon name="shield-alert" size={18} /></span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 4, flexWrap: "wrap" }}>
              <span style={{ font: "var(--fw-bold) 13px var(--font-body)", color: "var(--jv-cyan-300)" }}>{item.agent ?? "Agent"}</span>
              {irreversible && <span style={{ padding: "2px 7px", borderRadius: 3, font: "var(--fw-semibold) 9px var(--font-hud)", letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--jv-red)", background: "color-mix(in srgb, var(--jv-red) 13%, transparent)", border: "1px solid color-mix(in srgb, var(--jv-red) 32%, transparent)" }}>Irreversible</span>}
            </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 7, marginTop: 7, font: "var(--fw-medium) 11.5px var(--font-body)", color: "var(--jv-text-muted)" }}>
              <Icon name="lock" size={12} color="var(--jv-amber)" /><span style={{ color: "var(--jv-amber)" }}>{riskLabel(item.risk)}</span>{item.detail && <span>· {item.detail}</span>}
            </div>
          </div>
        </div>
        {sim && diff.length > 0 && (
...
export default function ApprovalsInbox() {
  const { data, reload } = useApi<Approval[]>("/api/approvals");
  const queue = data ?? [];
  ...
  const resolve = async (id: string, decision: ApprovalDecision, answer?: string) => {
    try {
      await api.post(`/api/approvals/${id}/resolve`, { decision, answer });
      flash(decision === "approved" ? "Action approved and fired." : decision === "answered" ? "Answer sent — agent resuming." : "Action rejected — agent notified.");
    } catch {
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto" }}>
      ...
      <ConfirmDialog open={clearing} danger title="Clear the approvals queue?" message="This removes every pending request from the inbox. Agents waiting on these decisions will be notified." confirmLabel="Clear all" onCancel={() => setClearing(false)} onConfirm={clearAll} />
    </div>
  );
}
